...
class TestProxyKmipClientIntegration(testtools.TestCase):
    ...
    def test_register_vendor_custom_get(self):
        """
        Test that a key with vendor attribute can be registered with the server and that its
        metadata is retrieved with the get operation.
        """
        key = objects.SymmetricKey(
            enums.CryptographicAlgorithm.AES,
            ...
            vendor_attribute=[
                {
                    'vendor_identification': 'Testing',
                    'attribute_name': 'Testing1',
                    'attribute_value': 'Testing2'
                },
                {
                    'vendor_identification': 'Testing3',
                    'attribute_name': 'Testing4',
                    'attribute_value': 'Testing5'
                }
            ]
        )
        key_id = self.client.register(key)
        attribute_list = self.client.get_attribute_list(key_id)
        self.assertIn('Vendor Attribute', attribute_list)
        result_id, attribute_list = self.client.get_attributes(
            uid=key_id,
            attribute_names=['Vendor Attribute']
        )
        self.assertEqual(key_id, result_id)
        ...
        self.assertEqual(
            'Testing',
            attribute.attribute_value.vendor_identification
        )
        self.assertEqual(
            'Testing1',
            attribute.attribute_value.attribute_name
        )
        self.assertEqual(
            'Testing2',
        ...
        self.assertEqual(
            'Testing3',
            attribute2.attribute_value.vendor_identification
        )
        self.assertEqual(
            'Testing4',
            attribute2.attribute_value.attribute_name
        )
        self.assertEqual(
            'Testing5',
            attribute2.attribute_value.attribute_value
        )
    ...
    def test_secret_data_register_get_destroy_vendor_custom(self):
        """
        Test that the ProxyKmipClient can register, retrieve, and destroy a
        secret with the vendor attribute field.
        """
        # Secret encoding obtained from Section 3.1.5 of the KMIP 1.1 test
        # documentation.
        secret = objects.SecretData(
